...
const AccountLoginPg = () => {
  ...
  const onClickSubmit = () => {
    const apiSucc = (res) => {
      ...
      if (success) {
        const { authToken, email, name, cellNumber, birthDate, socialLogin } =
          data;
        if (authToken?.accessToken) session.setToken(authToken?.accessToken);
        if (authToken?.refreshToken)
          session.setRefreshToken(authToken?.refreshToken);
  ...
  return (
    <LayoutCp
      headerTitle="로그인"
      button="close"
      onClickClose={() => navigate(ROUTES.LOGIN_MAIN)}
    >
      <section className="content_body">
        <div className="wrap">
          <dl className="login_title_dl">
            ...
            <dd className="FontS14">
              회원가입시 입력한 메일주소를 입력해 주세요.
            </dd>
          </dl>
